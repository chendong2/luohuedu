...
using(referenceModules, function () {
    ...
    var dataGridOptions = {
        columns: [[
            { field: 'TkId', checkbox: true },
            { field: 'OrderNum', title: '订单号', width: 150 },
            { field: 'TicketNo', title: '车票号', width: 150 },
            { field: 'LineName', title: '线路名称', width: 140 },
            { field: 'BeiYong3', title: '出售状态', width: 150 ,
                formatter: function (value, row, index) {
//                    alert(row.Phone);
                    if (row.Phone == "" && row.BeiYong2 =="")
                        return '<font color="red">未出售</font>';
                    else {
                        return '<font color="green">已出售</font>';
                    }
                }
            },
            { field: 'BeiYong4', title: '库存销售时间', width: 150 },
            ...
            { field: 'BeiYong1', title: '取票人姓名', width: 80 },
            { field: 'BeiYong2', title: '取票联系电话', width: 80 },
            { field: 'TicketPrice', title: '票价', width: 50 },
            { field: 'RealTicketPrice', title: '实际扣款价', width: 50 },
            { field: 'PaymentTypeStr', title: '付款类别', width: 80 },
            { field: 'PayAccount', title: '付款账户', width: 80 },
            { field: 'PayTimeStr', title: '付款时间', width: 130 },
            { field: 'IsRefund', title: '退票状态', width: 60, formatter: function (value) {
                if (value == 2)
                    return '<span style="color:red">已退票</span>';
                else
                    return '<font color="green">未退票</font>';
            }
            },
            { field: 'AuditState', title: '核票状态', width: 60, formatter: function (value) {
                if (value == 2)
                    return '<span style="color:green">已核票</span>';
                else
                    return '<font color="red">未核票</font>';
            }
            },
            { field: 'IsOutStr', title: '出票状态', width: 60, formatter: function (value) {
                if (value == "已出票")
                    return '<span style="color:green">已出票</span>';
                else
                    return '<font color="red">未出票</font>';
            }
            },
            { field: 'LeaveDateStr', title: '班次日期', width: 70, sortable: true },
            { field: 'LeaveTime', title: '班次时间', width: 60, sortable: true },
            { field: 'OnpointName', title: '上车点', width: 80 },
            { field: 'OffpointName', title: '下车点', width: 80 },
            { field: 'CurrencyType', title: '币种', width: 50 },
//            { field: 'Remark', title: '备注', width: 130 },
            
            { field: 'CreatedBy', title: '下单账号', width: 100 },
           
//            { field: 'OldTicketNo', title: '旧车票号', width: 120 },
        
            { field: 'OrderTypeStr', title: '车票类型', width: 60 },

            { field: 'StrOrderSource', title: '订单来源', width: 80, sortable: true },

            { field: 'CreatedOnStr', title: '下单日期', width: 150},
            { field: 'BeginCity', title: '出发城市', width: 80},
            { field: 'EndCity', title: '目的城市', width: 80},
//            { field: 'OldLeaveDateStr', title: '原班次日期', width: 70 },
//            { field: 'OldLeaveTime', title: '原班次时间', width: 70 },
//            { field: 'IsHangingStr', title: '挂账状态', width: 60 },
            { field: 'TicketSort', title: '流水号', width: 60 },
            { field: 'SeatNum', title: '座位号', width: 60 }

//            { field: 'RefundBy', title: '退票人', width: 60 },
//            { field: 'RefundOnStr', title: '退票时间', width: 120 },
