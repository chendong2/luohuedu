...
using(easyloader.defaultReferenceModules, function () {
    ...
    var dataGridOptions = {
        ...
        columns: [[
            ...
            { field: 'EducationtName', title: '对应类型', width: 180, sortable: false }
        ]],
        singleSelect: false,
        toolbar: '#toolbar',
        sortName: 'EducationtName',
        sortOrder: 'desc',
        rownumbers: true,
        pagination: true,
        loader: function (param, success, error) {
            var educationOfficeData = {
                page: param.page,
                rows: param.rows,
                order: param.order,
                sort: param.sort,
            ...
            var paramStr = JSON.stringify(educationOfficeData);

            ajaxCRUD({
                url: '/WebServices/Parameter/EducationOffice.asmx/GetEducationOfficeList',
                data: paramStr,
...
function loadPartialHtml() {
    if ($('.window').length == 0) {
        panel('formTemplate', {
            href: '/View/Parameter/EducationOffice/EducationOfficeForm.htm',
            onLoad: function () {
//                setValidatebox('Name', {
...
function fillForm(itemid) {
    ajaxCRUD({
        url: '/WebServices/Parameter/EducationOffice.asmx/GetEducationOfficeById',
        data: "{id:'" + itemid + "'}",
        success: function (data) {
            openDialog('dlg', {
                title: moduleName + '编辑',
...
function saveData() {
    ...
    var basicUrl = '/WebServices/Parameter/EducationOffice.asmx/';

    var wsMethod = '';
    if (hidValue.length > 0) {
        wsMethod = "UpdateEducationOffice"; //修改
    } else {
        wsMethod = "AddEducationOffice"; //新增
    }
    ...
    var jsonDataStr = "{educationOfficeBo:" + form2JsonStr + "}";
...
function deleteDatasAjax(str) {
    ajaxCRUD({
        url: '/WebServices/Parameter/EducationOffice.asmx/DeleteEducationOfficesByIds',
        data: "{ids:'" + str + "'}",
        success: function (data) {
            if (data == true) {
    ...
    var dataGridOptions = {
        ...
        loader: function (param, success, error) {
            var educationOfficeData = {
                ...
                sort: param.sort,
                educationOfficeBo: {
                    EducationtName: $("#txtEducationtName").val().trim()
                }
            };
            var paramStr = JSON.stringify(educationOfficeData);
