...
//添加布尔型参数isAutoFilter，是否支持手动输入并智能过滤
...
$.extend($.fn.validatebox.defaults.rules, {
    ...
    uniqueTwoParam: {
        validator: function (value, param) {
            value = $.trim(value);
            var serviceUrl = param[0];
            var idVal = $('#' + param[1]).val();
            var oldValue1 = $.trim($('#' + param[2]).val());
            var paramName1 = param[3];
            var oldValue2 = $.trim($('#' + param[4]).val());
            var paramName2 = param[5];
            var newValue2 = $.trim($('#' + param[6]).val());
            var dataStr = '{' + paramName1 + ':"' + value + '",' + paramName2 + ':"' + newValue2 + '"}';
            var result = false;
            if (newValue2.length == 0) { return true; }
            if (idVal.length > 0 && oldValue1 == value
                && newValue2.length > 0 && oldValue2 == newValue2) {
                return true;
            }
