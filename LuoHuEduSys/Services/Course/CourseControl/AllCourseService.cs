        ...
        public Page<CourseBo> GetPerStudents(int page, int rows, string order, string sort, CourseBo courseBo, string studentId)
        {
            ...
            string strSql = string.Format(@"SELECT c.*,s.SubjectName,t.TrainType,sh.`SchoolName`, CONCAT(st.Name,c.`WaiPingName`) AS NAME,COUNT(ct.id) AS YiBao    FROM                                                                                tb_course AS c 
                                            ...
                                            LEFT JOIN `tb_coursestudent` ct ON ct.`CourseId`=c.`Id`
                                             WHERE  c.Locked=2  AND DATE_SUB( DATE_FORMAT( c.TimeEnd,'%Y-%m-%d'),INTERVAL 1 DAY) >DATE_SUB( DATE_FORMAT(NOW(),'%Y-%m-%d'),INTERVAL 1 DAY) AND CourseState=2 AND EXISTS( SELECT * FROM tb_student st LEFT JOIN tb_subject su ON st.`FirstTeaching`=su.`Id`  
